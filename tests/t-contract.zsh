#!/usr/bin/env zunit
#{{{                    MARK:Header
##### Purpose: zsh-git-repo-cache — plugin-contract pins.
...
@test 'entrypoint stem matches plugin directory basename' {
    ...
    # stem of `zsh-git-repo-cache.plugin.zsh` must equal the parent directory's
    # basename so generated source lines stay copy-pasteable.
    local entry='zsh-git-repo-cache.plugin.zsh'
...
@test 'entrypoint parses cleanly under zsh -n' {
    run zsh -n "$pluginDir/zsh-git-repo-cache.plugin.zsh"
    assert $state equals 0
}
